...
function buildVolumeProfile(profile, race, block) {
  ...
  // A1 safety guardrail: a long run must never dominate the week. Cap every long
  // run at 45% of that week's volume so low-mileage plans can't yield a 34km run
  // on a 45km week.
  const LONG_RUN_MAX_FRAC = 0.45;
  for (let i = 0; i < n; i++) {
    if (blk[i].phase === "Taper" || blk[i].phase === "Race" || blk[i].recovery) continue;
    longs[i] = Math.min(longs[i], Math.max(10, Math.round(Math.min(weekly[i], curVol * 1.4) * LONG_RUN_MAX_FRAC)));
  }
  // Marathon 30km+/32km guarantees only unlock when weekly volume supports them.
  let longRunNote = null;
  if (dist === "Marathon" && curVol >= 50) {
    ...
    longRunNote = "Long runs are capped near 45% of weekly volume. Build weekly mileage toward ~55-65 km to safely unlock 30 km+ long runs.";
  }
...
function generatePlan(profile, race, holidays = [], bRaces = []) {
  ...
  const block = buildBlockMeta(blockWeeks, race.type);
  ...
  return { planStart: iso(week0Monday), trainingStart: planStartActual, raceDate: raceISO, totalWeeks, blockWeeks, weeks, raceWeekIndex: blockWeeks - 1, race, longRunNote: vol.longRunNote };
}
...
function deriveState(core) {
  ...
  const goal = predictGoal(race, load, fitness, completionPct, longRunPct, recovery.score);
  ...
  const carb = (race.type === "Marathon" || race.type === "Half") ? carbLoad(profile) : null;
...
function seedCore() {
  ...
  return {
    ...
    health: { illness: false, injury: false }, overrides: {}, completions: {}, runLogs: [], liftLogs: hist.lifts, bodyLogs: hist.body, sessionLogs: [], vo2Logs: [],
  };
}
function emptyCore() {
  ...
  return {
    ...
    health: { illness: false, injury: false }, overrides: {}, completions: {}, runLogs: [], liftLogs: [], bodyLogs: [], sessionLogs: [], vo2Logs: [],
  };
}
