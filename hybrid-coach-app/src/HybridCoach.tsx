...
function buildVolumeProfile(profile, race, block) {
  ...
  // Guarantee marathon long-run constraints regardless of starting volume
  if (dist === "Marathon") {
    const cand = blk.map((m, i) => (!m.recovery && (m.phase === "Build" || m.phase === "Peak")) ? i : -1).filter((i) => i >= 0).sort((a, b) => longs[b] - longs[a]);
    const top = cand[0];
    longs[top] = Math.max(32, Math.min(35, longs[top] < 32 ? 34 : longs[top]));
    let over30 = longs.filter((x, i) => !blk[i].recovery && blk[i].phase !== "Taper" && blk[i].phase !== "Race" && x >= 30).length;
    for (let j = 1; j < cand.length && over30 < 2; j++) { if (longs[cand[j]] < 30) { longs[cand[j]] = 30; over30++; } }
    if (!longs.some((x, i) => !blk[i].recovery && x >= 32)) longs[top] = Math.max(longs[top], 32);
  }
  return { curVol, frac, longs, weekly, peakLong };
}
...
const METHOD_QUALITY = { Polarised: ["Intervals", "Threshold"], Pyramidal: ["Tempo", "Threshold"], Threshold: ["Threshold", "Threshold"], Norwegian: ["Threshold", "Threshold"] };
...
const HR_ZONE = { "Recovery Run": ["Z1", 0.60, 0.70], "Easy Run": ["Z2", 0.70, 0.78], "Long Run": ["Z2", 0.73, 0.80], Tempo: ["Z3", 0.80, 0.87], Threshold: ["Z4", 0.87, 0.92], Intervals: ["Z5", 0.92, 0.97], Race: ["Z3-4", 0.82, 0.90], Travel: ["Z2", 0.68, 0.78] };
...
function buildWorkoutData(type, phase, longKm, recovery, maxHr, easyKm) {
  ...
  switch (type) {
    ...
    case "Tempo": return mk({ name: "Tempo", dist: 10, dur: 55, int: "moderate", load: 80, detail: "25 min sustained at marathon effort", intervals: { reps: 1, mode: "time", workMin: 25, recovery: "continuous", paceZone: "Marathon" } });
    case "Threshold": return mk({ name: "Threshold", dist: 12, dur: 65, int: "high", load: 108, detail: "4 x 6 min @ threshold, 90s float", intervals: { reps: 4, mode: "time", workMin: 6, recovery: "90s float", paceZone: "Threshold" } });
    case "Intervals": return mk({ name: "VO2 intervals", dist: 10, dur: 58, int: "high", load: 100, detail: "6 x 800m @ 5k effort, equal recovery", intervals: { reps: 6, mode: "dist", workM: 800, recovery: "equal recovery", paceZone: "Interval (VO2)" } });
...
const isHard = (t) => t === "Intervals" || t === "Threshold";
function weekSessions(phase, recovery, split, methodology) {
  ...
  let qualities = [];
  if (phase === "Base" || phase === "Taper") qualities = ["Tempo"];
...
function weekPenalty(types, phase) {
  ...
  const quality = (t) => t === "Intervals" || t === "Threshold" || t === "Tempo" || t === "Race" || t === "Long Run";
...
function generatePlan(profile, race, holidays = [], bRaces = []) {
  ...
  const raceISO = iso(new Date(race.date));
  const raceWeekStart = mondayOf(raceISO);
  let blockWeeks = Math.round(diffDays(raceWeekStart, week0Monday) / 7) + 1;
  blockWeeks = Math.max(3, Math.min(51, blockWeeks));
  const block = buildBlockMeta(blockWeeks);
  const vol = buildVolumeProfile(profile, race, block);
  const f = splitFor(profile.availabilityDays); // run/strength split by days/week
  const raceDistKm = (GOAL_DIST_M[race.type] || 42195) / 1000;
  for (let w = 0; w < totalWeeks; w++) {
    const inBlock = w < blockWeeks;
    const meta = inBlock ? block[w] : { phase: "Off-Season", recovery: false };
    const longKm = inBlock ? vol.longs[w] : 16;
    const weeklyTarget = inBlock ? vol.weekly[w] : Math.round(vol.curVol * 0.8);
  ...
  return { planStart: iso(week0Monday), trainingStart: planStartActual, raceDate: raceISO, totalWeeks, blockWeeks, weeks, raceWeekIndex: blockWeeks - 1, race };
}
...
function generateStrengthSession(profile, weekNumber, phase, isRecovery, sessionParity = 0, weights = {}) {
  ...
  const sessionA = sessionParity === 0;
  const compounds = sessionA
    ? [{ name: "Romanian Deadlift", pattern: "Hinge", oneRM: rm.romanianDeadlift }, { name: "Bench Press", pattern: "Push", oneRM: rm.benchPress }]
    : [{ name: "Back Squat", pattern: "Squat", oneRM: rm.backSquat }, { name: "Overhead Press", pattern: "Push", oneRM: rm.overheadPress }];
  const compound = compounds.map((c) => ({ name: c.name, pattern: c.pattern, sets: scheme.sets, reps: scheme.reps, weightKg: round2_5((c.oneRM || 60) * scheme.pct * overload), pctOneRM: Math.round(scheme.pct * overload * 100) }));
  // Two complementary accessory sets so the week is well-rounded - each session still
  // covers hinge, single-leg, vertical pull and horizontal push, but with different lifts.
  const accBase = sessionA
    ? [
        { name: "Hip Thrust", pattern: "Hinge", category: "glute-hinge", sets: 3, reps: 10, note: "glute drive" },
        { name: "Bulgarian Split Squat", pattern: "Single-leg", category: "lunge", sets: 3, reps: 8, note: "per leg, DB" },
        { name: "Weighted Pull-up", pattern: "Vertical pull", category: "vertical-pull", sets: 3, reps: 6, note: "added load" },
        { name: "Incline DB Press", pattern: "Horizontal push", category: "horizontal-push", sets: 3, reps: 10, note: "per hand" },
      ]
    : [
        { name: "Back Extension", pattern: "Hinge", category: "glute-hinge", sets: 3, reps: 12, note: "posterior chain" },
        { name: "Walking Lunge", pattern: "Single-leg", category: "lunge", sets: 3, reps: 10, note: "per leg, DB" },
        { name: "Lat Pulldown", pattern: "Vertical pull", category: "vertical-pull", sets: 3, reps: 10, note: "controlled" },
        { name: "Dips", pattern: "Horizontal push", category: "horizontal-push", sets: 3, reps: 8, note: "bodyweight + added load" },
      ];
  const accessory = accBase.map((a) => ({ ...a, weightKg: (weights[a.name] != null ? weights[a.name] : (ACCESSORY_START[a.name] ?? 20)) }));
  const prehab = sessionA
    ? [{ name: "Banded Glute Bridge", sets: 2, reps: 15 }, { name: "Single-leg Calf Raise", sets: 2, reps: 15, note: "per leg" }, { name: "Copenhagen Plank", sets: 2, reps: "30s", note: "per side" }]
    : [{ name: "Dead Bug", sets: 2, reps: 12, note: "per side" }, { name: "Banded Lateral Walk", sets: 2, reps: 15, note: "per side" }, { name: "Side Plank", sets: 2, reps: "30s", note: "per side" }];
  return { scheme: scheme.label, overloadPct: Math.round((overload - 1) * 100), session: sessionA ? "A" : "B", prehab, compound, accessory };
}
...
function computeNutrition(profile, workout, phase, recoveryStatus, isRecoveryWeek, weightCtx) {
  ...
  const hydWork = { Rest: 0, "Easy Run": 500, "Recovery Run": 350, Tempo: 700, Threshold: 800, Intervals: 700, "Long Run": 1300, Strength: 300, Race: 1600, Travel: 300 }[workout.type] ?? 0;
...
function raceFuelling(profile, projSecs) {
  ...
  return { carbPerHr, gelCarbs, totalCarb, totalGels, intervalMin, fluidPerHr: 600, sodiumPerHr: 500, hours: hours.toFixed(2), schedule };
}

/* ---- PREDICTIONS / VO2 / WEIGHT ---- */
function predictGoal(race, load, completionPct, longRunPct, recoveryScore) {
  ...
  const gap = 0.9 - confidence / 100; const projSecs = Math.round(goalSecs * (1 + gap * 0.06)); const range = Math.round(goalSecs * 0.018);
...
function pacingStrategies(projSecs, raceKm = 42.195) {
  const mp = projSecs / raceKm; // sec per km
  const mk = (step) => { const pts = []; for (let km = step; km < raceKm - 0.05; km += step) pts.push({ km: km + "k", time: secsToHMS(mp * km) }); pts.push({ km: "Finish", time: secsToHMS(mp * raceKm) }); return pts; };
  const half = raceKm / 2;
  return { goalPace: pace(mp), raceKm: Math.round(raceKm * 10) / 10, splits1k: mk(1), splits5k: mk(5), evenSplits: mk(5), negative: { firstHalf: secsToHMS((mp + 6) * half), secondHalf: secsToHMS((mp - 6) * half) } };
}
...
function deriveState(core) {
  const today = core.today, profile = core.profile;
  const races = core.races || [];
  const race = races.find((r) => r.priority === "A") || races[0];
  ...
  let plan = generatePlan({ ...profile, methodology }, race, core.holidays, bRaces);
  ...
  const goal = predictGoal(race, load, completionPct, longRunPct, recovery.score);
  const readiness = raceReadiness(load, recovery.score, longRunPct, completionPct, core.recoveryInputs.sleepQuality);
  ...
  const raceCountdown = diffDays(race.date, today);
  const weeksToRace = Math.max(0, Math.round(raceCountdown / 7));
  const weight = predictRaceWeight(profile, weeksToRace);
  ...
  const weightCtx = { kgToLose: weight.aboveRange, daysToTarget: daysToRaceWeight, endOfBuildISO, trendKgPerWeek: weightTrend };
  const nutrition = computeNutrition(profile, todayWorkout, curWeek.phase, recovery.status, curWeek.isRecovery, weightCtx);
  const alerts = buildAlerts(recovery, load, readiness, missedKey, holidayActive, shoeAlerts);
  const fitness = estimateFitness(load, vo2ForPaces);
  const paces = computePaces(fitness);
  const predictions = racePredictions(goal.projectedSeconds);
  const pacing = pacingStrategies(goal.projectedSeconds, (GOAL_DIST_M[race.type] || 42195) / 1000);
  const fuelling = raceFuelling(profile, goal.projectedSeconds);
  const carb = carbLoad(profile);
  ...
  const strengthDays = allDays(plan).filter((d) => d.type === "Strength").map((d) => d.date);
  const sessionParity = strengthDays.filter((dt) => dt < todayISO).length % 2;
  const strengthSession = generateStrengthSession(profile, curWeek.weekNumber, curWeek.phase, curWeek.isRecovery, sessionParity, core.liftWeights || {});
  ...
  const coachReview = buildCoachReview({ phase: curWeek.phase, week: curWeek.weekNumber, weeksToRace, completionPct, longRunPct, hasLongRuns: longTotal > 0, vo2Trend, recovery, readiness, load, missedKey, holidayActive, isRecoveryWeek: curWeek.isRecovery, weight, shoeAlerts });
  const goalDist = GOAL_DIST_M[race.type] || 42195;
  const vo2Target = danielsVO2(goalDist, race.goalSeconds);
  ...
  const todayState = {
    ...
    nutritionTargets: nutrition, hydrationTarget: nutrition.hydration, raceCountdown, raceReadiness: readiness,
    goalConfidence: goal.confidence, goalForecast: goal, coachAlerts: alerts, upcomingEvents: core.races.map((r) => ({ ...r, daysAway: diffDays(r.date, today) })),
    completionPct, longRunPct, maxHr, paces, fitness, predictions, weight, pacing, fuelling, carb, strengthSession, progress, weeksToRace, holidayActive, vo2Target, vo2Current,
...
function seedCore() {
  ...
  return {
    ...
    completions: {}, runLogs: [], liftLogs: hist.lifts, bodyLogs: hist.body, sessionLogs: [], vo2Logs: [],
  };
}
function emptyCore() {
  ...
  return {
    ...
    completions: {}, runLogs: [], liftLogs: [], bodyLogs: [], sessionLogs: [], vo2Logs: [],
  };
}
function reducer(core, action) {
  switch (action.type) {
    ...
    case "RunLogged": {
      const aRace = (core.races || []).find((r) => r.priority === "A") || core.races[0];
      ...
      return { ...core, shoes, runLogs: [{ ...action.run, date: action.date }, ...core.runLogs], vo2Logs, sessionLogs: (action.run.splits || action.run.reps) ? [{ kind: "run", date: action.date, type: action.run.type, splits: action.run.splits, reps: action.run.reps }, ...core.sessionLogs] : core.sessionLogs, completions: { ...core.completions, [action.date]: { status: v.compliance === "Missed" ? "Missed" : "Completed", compliance: v.compliancePct / 100, key: ["Long Run", "Threshold", "Intervals", "Race"].includes(planned?.type) } } };
    }
    ...
    case "LiftLogged": {
      ...
      return { ...core, profile: { ...core.profile, oneRM }, liftLogs: [{ ...action.lift, e1rm, date: action.date }, ...core.liftLogs] };
    }
    case "StrengthLogged": {
      ...
      action.exercises.forEach((ex) => {
        ...
        liftLogs.unshift({ exercise: ex.name, weightKg: top.weight, reps: top.reps, sets, e1rm: best, date: action.date });
      });
      return { ...core, profile: { ...core.profile, oneRM }, liftWeights, liftLogs, sessionLogs: [{ kind: "strength", date: action.date, notes: action.notes, exercises: action.exercises }, ...core.sessionLogs], completions: { ...core.completions, [action.date]: { status: "Completed", compliance: 1, key: false } } };
    }
    case "CheckinSaved": return { ...core, recoveryInputs: { ...core.recoveryInputs, ...action.inputs }, lastCheckinDate: action.date };
    case "RecoverySaved": return { ...core, recoveryInputs: { ...core.recoveryInputs, ...action.inputs } };
    case "BodyLogged": { const measurements = action.measurements ? { ...core.profile.measurements, ...action.measurements } : core.profile.measurements; const bf = (action.bodyFatPct && action.bodyFatPct > 0) ? action.bodyFatPct : (navyBodyFat({ ...core.profile, measurements }) ?? core.profile.bodyFatPct); return { ...core, profile: { ...core.profile, weightKg: action.weightKg, bodyFatPct: bf, measurements }, bodyLogs: [...core.bodyLogs, { date: action.date, weightKg: action.weightKg, bodyFatPct: bf }] }; }
...
function Dashboard({ T, dispatch, go, goTab }) {
  const w = T.todaysWorkout, WIcon = ICONS[w.type] || Activity, n = T.nutritionTargets;
  ...
  return (<div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
    ...
    <Card pad={14}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
      ...
      <div style={{ flex: 1, textAlign: "center" }}><Eyebrow>A race in</Eyebrow><div style={{ fontSize: 18, fontWeight: 800, fontVariantNumeric: "tabular-nums", marginTop: 2 }}>{T.raceCountdown}<span style={{ fontSize: 11, color: C.faint }}> d</span></div></div>
    ...
    <Card style={{ position: "relative", background: `linear-gradient(135deg, ${C.surface2}, ${C.surface})`, borderColor: intensityColor(w.intensity) + "66" }}>
      ...
      <div onClick={() => go({ name: "workout", payload: w })} style={{ cursor: "pointer" }}>
        ...
        <div style={{ display: "flex", gap: 24, marginTop: 14, flexWrap: "wrap" }}>{w.distanceKm > 0 && <Stat label="Distance" value={w.distanceKm} unit="km" />}{w.durationMin > 0 && <Stat label="Duration" value={w.durationMin} unit="min" />}<Stat label="Load" value={w.load} color={intensityColor(w.intensity)} />{w.hrZone && <Stat label="HR zone" value={w.hrZone.zone} color={hrColor(w.hrZone.zone)} />}</div>
        {w.hrZone && <div style={{ color: C.faint, fontSize: 12, marginTop: 6 }}>Target HR {w.hrZone.lo}-{w.hrZone.hi} bpm</div>}
      </div>
      {w.type !== "Rest" && (<div style={{ display: "flex", gap: 8, marginTop: 14 }}>
        <button onClick={() => go({ name: "workout", payload: w })} style={btn(C.teal)}><ClipboardList size={15} /> Log workout</button>
        <button onClick={() => dispatch({ type: "WorkoutCompleted", date: T.currentDate, key: ["Long Run", "Threshold", "Intervals", "Race"].includes(w.type), compliancePct: 100 })} style={btn(C.sky, true)}><CheckCircle2 size={15} /> Done</button>
        <button onClick={() => dispatch({ type: "WorkoutMissed", date: T.currentDate, key: ["Long Run", "Threshold", "Intervals", "Race"].includes(w.type) })} style={btn(C.rose, true)}>Missed</button>
      </div>)}
    </Card>
    ...
    <div style={{ display: "flex", gap: 14 }}>
      <Card style={{ position: "relative", flex: 1, cursor: "pointer" }}><Corner onClick={() => goTab("race")} /><div onClick={() => goTab("race")}><div style={{ display: "flex", alignItems: "center", gap: 6 }}><Trophy size={14} color={C.amber} /><Eyebrow>Upcoming race</Eyebrow></div><div style={{ fontSize: 15, fontWeight: 800, marginTop: 8 }}>{T.upcomingEvents[0]?.name}</div><div style={{ color: C.dim, fontSize: 12, marginTop: 2 }}>{fmtShort(T.upcomingEvents[0]?.date)} - Goal {T.upcomingEvents[0]?.goalTime}</div><div style={{ marginTop: 10 }}><Pill color={STATUS_COLOR[T.raceReadiness.category]}>Readiness {T.raceReadiness.score} - {T.raceReadiness.category}</Pill></div></div></Card>
      <Card style={{ position: "relative", flex: 1, cursor: "pointer" }}><Corner onClick={() => goTab("race")} /><div onClick={() => goTab("race")}><div style={{ display: "flex", alignItems: "center", gap: 6 }}><Target size={14} color={C.violet} /><Eyebrow>Goal forecast</Eyebrow></div><div style={{ fontSize: 30, fontWeight: 800, marginTop: 8, fontVariantNumeric: "tabular-nums" }}>{T.goalConfidence}<span style={{ fontSize: 14, color: C.faint }}>%</span></div><div style={{ color: C.dim, fontSize: 12, marginTop: 2 }}>confidence - {T.upcomingEvents[0]?.goalTime}</div><div style={{ marginTop: 8, fontSize: 13, fontWeight: 700 }}>Projected {T.goalForecast.projectedTime}</div></div></Card>
    </div>
  ...
  return (<div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
    ...
    <Card pad={16}>
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
        <div style={{ flex: 1, background: C.surface2, borderRadius: 10, padding: "10px 8px", textAlign: "center" }}><div style={{ fontSize: 19, fontWeight: 800, color: C.sky, fontVariantNumeric: "tabular-nums" }}>{week.volumeKm}</div><div style={{ fontSize: 10.5, color: C.faint, textTransform: "uppercase", letterSpacing: 0.6, marginTop: 2 }}>km volume</div></div>
        <div style={{ flex: 1, background: C.surface2, borderRadius: 10, padding: "10px 8px", textAlign: "center" }}><div style={{ fontSize: 19, fontWeight: 800, color: C.teal, fontVariantNumeric: "tabular-nums" }}>{week.longRunKm}</div><div style={{ fontSize: 10.5, color: C.faint, textTransform: "uppercase", letterSpacing: 0.6, marginTop: 2 }}>long run</div></div>
...
function RunLogForm({ w, dispatch, date, isInterval, shoes = [], paces }) {
  ...
  return (<Card><Eyebrow>Log this run</Eyebrow><div style={{ marginTop: 10 }}>
    <Field label="Total distance (km)" value={d.distanceKm} step="0.1" onChange={(v) => setD({ ...d, distanceKm: v })} />
    ...
    <div style={{ marginBottom: 12 }}><div style={{ color: C.dim, fontSize: 12, marginBottom: 4 }}>Shoe (tracks lifetime mileage)</div>
      {active.length ? (<select value={d.shoeId} onChange={(e) => setD({ ...d, shoeId: e.target.value })} style={inputStyle}><option value="">- none -</option>{active.map((s) => <option key={s.id} value={s.id}>{s.name} ({Math.round(s.km)} km)</option>)}</select>) : (<div style={{ color: C.faint, fontSize: 12 }}>No shoes yet - add a pair in Profile - Shoe tracker to log mileage.</div>)}
    </div>
...
function StrengthSessionLog({ ss, dispatch, date }) {
  ...
  const items = [
    ...
    ...ss.compound.map((c) => ({ group: "Compound", color: C.ember, name: c.name, sets: c.sets, reps: c.reps, weightKg: c.weightKg, presc: `${c.sets} x ${c.reps} @ ${c.weightKg}kg - ${c.pctOneRM}% 1RM (${c.pattern})`, alts: null })),
    ...ss.accessory.map((a) => ({ group: "Accessory", color: C.amber, name: a.name, sets: a.sets, reps: a.reps, weightKg: a.weightKg, presc: `${a.sets} x ${a.reps} @ ${a.weightKg}kg - ${a.pattern}${a.note ? " - " + a.note : ""}`, alts: EXERCISE_ALTS[a.category] || null })),
  ...
  const [ex, setEx] = useState(items.map((it) => it.timed
    ...
    : { name: it.name, timed: false, sets: Array.from({ length: it.sets }, () => ({ weight: it.weightKg || 0, reps: repNum(it.reps) })) }));
  ...
  const submit = () => {
    const exercises = ex.map((e, i) => ({ name: e.name, timed: e.timed, sets: e.sets, target: (e.timed || items[i].group === "Compound") ? null : { sets: items[i].sets, reps: repNum(items[i].reps), weightKg: items[i].weightKg } }));
...
function WorkoutDetails({ T, workout, dispatch, onBack }) {
  const w = workout, col = intensityColor(w.intensity), Icon = ICONS[w.type] || Activity, ss = T.strengthSession;
  ...
  return (<div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
    ...
    <Card style={{ borderColor: col + "55" }}>
      ...
      <div style={{ display: "flex", gap: 22, marginTop: 14, flexWrap: "wrap" }}>{w.distanceKm > 0 && <Stat label="Distance" value={w.distanceKm} unit="km" />}{w.durationMin > 0 && <Stat label="Duration" value={w.durationMin} unit="min" />}<Stat label="Load" value={w.load} color={col} />{w.hrZone && <Stat label="HR zone" value={w.hrZone.zone} color={hrColor(w.hrZone.zone)} />}</div>
...
function RaceView({ T, dispatch, go }) {
  const ev = T.upcomingEvents[0], g = T.goalForecast, f = T.fuelling;
  ...
  return (<div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
    ...
    <Card><button onClick={() => setFuelOpen((o) => !o)} style={{ cursor: "pointer", width: "100%", background: "transparent", border: "none", padding: 0, display: "flex", alignItems: "center", justifyContent: "space-between" }}><span style={{ display: "flex", alignItems: "center", gap: 6 }}><Flame size={14} color={C.sky} /><Eyebrow>Race fuelling & carb loading</Eyebrow></span><ChevronRight size={18} color={C.faint} style={{ transform: fuelOpen ? "rotate(90deg)" : "none", transition: "transform 0.15s" }} /></button>
      ...
      {fuelOpen && (<>
        ...
        <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 6 }}><Row left="Carb loading" right={`${T.carb.dailyGrams} g/day`} sub={`${T.carb.days} days - ${T.carb.perKg} g/kg - ${T.carb.totalGrams} g total`} /><Row left="Fluid" right={`${f.fluidPerHr} ml/hr`} /><Row left="Sodium" right={`${f.sodiumPerHr} mg/hr`} /></div>
      </>)}</Card>
...
function BodyLogForm({ core, dispatch }) {
  ...
  return (<Card><Eyebrow>Log body composition</Eyebrow><div style={{ marginTop: 10 }}><Field label="Weight (kg)" value={d.weightKg} step="0.1" onChange={(v) => setD({ ...d, weightKg: v })} /><Field label="Body fat (%)" value={d.bodyFatPct} step="0.1" onChange={(v) => setD({ ...d, bodyFatPct: v })} /><button onClick={() => dispatch({ type: "BodyLogged", date: iso(new Date()), weightKg: d.weightKg, bodyFatPct: d.bodyFatPct })} style={btn(C.teal)}><Plus size={15} /> Log measurement</button></div></Card>);
}
function ProgressView({ T, core, dispatch }) {
  const [tab, setTab] = useState("Running"); const p = T.progress;
  ...
  return (<div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
    ...
    {tab === "Running" && (<>
      <Card><Eyebrow>Weekly running volume</Eyebrow><div style={{ height: 180, marginTop: 12 }}><ResponsiveContainer width="100%" height="100%"><BarChart data={p.running} margin={{ top: 4, right: 6, bottom: 0, left: -24 }}><CartesianGrid stroke={C.line} strokeDasharray="3 3" vertical={false} /><XAxis dataKey="week" tick={{ fill: C.faint, fontSize: 10 }} axisLine={false} tickLine={false} /><YAxis tick={{ fill: C.faint, fontSize: 10 }} axisLine={false} tickLine={false} /><Tooltip contentStyle={{ background: C.surface2, border: `1px solid ${C.line}`, borderRadius: 8, fontSize: 12 }} /><Bar dataKey="volume" name="km" fill={C.ember} radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer></div></Card>
      <Card><Eyebrow>Long-run progression</Eyebrow><div style={{ height: 180, marginTop: 12 }}><ResponsiveContainer width="100%" height="100%"><AreaChart data={p.longRun} margin={{ top: 4, right: 6, bottom: 0, left: -24 }}><defs><linearGradient id="lr" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={C.teal} stopOpacity={0.35} /><stop offset="100%" stopColor={C.teal} stopOpacity={0} /></linearGradient></defs><CartesianGrid stroke={C.line} strokeDasharray="3 3" vertical={false} /><XAxis dataKey="week" tick={{ fill: C.faint, fontSize: 10 }} axisLine={false} tickLine={false} /><YAxis tick={{ fill: C.faint, fontSize: 10 }} axisLine={false} tickLine={false} /><Tooltip contentStyle={{ background: C.surface2, border: `1px solid ${C.line}`, borderRadius: 8, fontSize: 12 }} /><Area type="monotone" dataKey="km" name="long run km" stroke={C.teal} fill="url(#lr)" strokeWidth={2} /></AreaChart></ResponsiveContainer></div></Card>
    ...
    {tab === "Body" && (<><Card><Eyebrow>Body composition trend</Eyebrow><div style={{ height: 220, marginTop: 12 }}><ResponsiveContainer width="100%" height="100%"><LineChart data={p.body} margin={{ top: 4, right: 6, bottom: 0, left: -24 }}><CartesianGrid stroke={C.line} strokeDasharray="3 3" vertical={false} /><XAxis dataKey="date" tick={{ fill: C.faint, fontSize: 10 }} axisLine={false} tickLine={false} /><YAxis yAxisId="w" domain={["dataMin - 1", "dataMax + 1"]} tick={{ fill: C.faint, fontSize: 10 }} axisLine={false} tickLine={false} /><YAxis yAxisId="bf" orientation="right" tick={{ fill: C.faint, fontSize: 10 }} axisLine={false} tickLine={false} /><Tooltip contentStyle={{ background: C.surface2, border: `1px solid ${C.line}`, borderRadius: 8, fontSize: 12 }} /><Line yAxisId="w" dataKey="weight" name="Weight" stroke={C.teal} dot={false} strokeWidth={2} /><Line yAxisId="bf" dataKey="bf" name="Body fat" stroke={C.amber} dot={false} strokeWidth={2} strokeDasharray="4 3" /></LineChart></ResponsiveContainer></div></Card>
      <Card><Eyebrow>Lean mass trend</Eyebrow><div style={{ height: 180, marginTop: 12 }}><ResponsiveContainer width="100%" height="100%"><AreaChart data={leanSeries} margin={{ top: 4, right: 6, bottom: 0, left: -24 }}><defs><linearGradient id="lean" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={C.sky} stopOpacity={0.35} /><stop offset="100%" stopColor={C.sky} stopOpacity={0} /></linearGradient></defs><CartesianGrid stroke={C.line} strokeDasharray="3 3" vertical={false} /><XAxis dataKey="date" tick={{ fill: C.faint, fontSize: 10 }} axisLine={false} tickLine={false} /><YAxis domain={["dataMin - 1", "dataMax + 1"]} tick={{ fill: C.faint, fontSize: 10 }} axisLine={false} tickLine={false} /><Tooltip contentStyle={{ background: C.surface2, border: `1px solid ${C.line}`, borderRadius: 8, fontSize: 12 }} /><Area type="monotone" dataKey="lean" name="lean kg" stroke={C.sky} fill="url(#lean)" strokeWidth={2} /></AreaChart></ResponsiveContainer></div><div style={{ color: C.faint, fontSize: 11.5, marginTop: 6 }}>Holding lean mass while losing weight means you're losing fat, not muscle - exactly what you want into race weight.</div></Card>
...
function ProfileView({ core, dispatch, go, user = null, onChangePassword = null, onDeleteAccount = null }) {
  ...
  return (<div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
    ...
    <Card><div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}><div><div style={{ fontSize: 20, fontWeight: 800 }}>{p.name || "Unnamed athlete"}</div><div style={{ color: C.dim, fontSize: 12.5, marginTop: 2 }}>{p.age} - {p.sex} - {p.heightCm} cm</div></div><button onClick={() => go({ name: "setup" })} style={btn(C.ember, true)}><Settings size={14} /> Edit</button></div>
      <div style={{ display: "flex", gap: 20, marginTop: 14, flexWrap: "wrap" }}><Stat label="Weight" value={p.weightKg} unit="kg" /><Stat label="Body fat" value={bf} unit="%" color={C.amber} /><Stat label="Max HR" value={p.maxHr} unit="bpm" color={C.rose} /><Stat label="Volume" value={p.currentWeeklyVolumeKm} unit="km/wk" color={C.sky} /></div>
      <div style={{ color: C.faint, fontSize: 11, marginTop: 6 }}>Body fat {bfSrc}. 1RMs: Sq {p.oneRM.backSquat} - RDL {p.oneRM.romanianDeadlift} - Bench {p.oneRM.benchPress} - OHP {p.oneRM.overheadPress} kg.</div>
    </Card>
    <Card><Eyebrow>Goal race</Eyebrow><div style={{ marginTop: 8 }}><Row left={aRace.name} right={aRace.goalTime} sub={`${aRace.type} - ${fmtShort(aRace.date)}`} color={C.amber} /></div>{bRaces.length > 0 && <div style={{ color: C.faint, fontSize: 11.5, marginTop: 6 }}>{bRaces.length} B-race{bRaces.length === 1 ? "" : "s"} woven into the plan.</div>}</Card>
    <Card><Eyebrow>Configuration</Eyebrow><div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      <NavRow icon={Beaker} label="Core logic tests" color={C.teal} onClick={() => go({ name: "tests" })} />
    </div></Card>
...
function SettingsView({ core, dispatch, onBack }) {
  const p = core.profile, set = (patch) => dispatch({ type: "ProfileUpdated", profile: patch });
  ...
  return (<div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
    ...
    <Card>
      ...
      <div style={{ color: C.faint, fontSize: 11.5, marginTop: -6, marginBottom: 10 }}>Running-focused marathon + strength. Runs / strength by days: 3 - 3 + 0 - 4 - 3 + 1 - 5 - 4 + 1 - 6 - 4 + 2 - 7 - 5 + 2. Remaining days are rest.</div>
      <Field label="Long run day" value={p.longRunDay} options={["Sun", "Sat", "Wed", "Mon"]} onChange={(v) => set({ longRunDay: v })} />
      <Field label="Current weekly running volume (km)" value={p.currentWeeklyVolumeKm} onChange={(v) => set({ currentWeeklyVolumeKm: v })} />
    ...
    <Card><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><Eyebrow>Training methodology</Eyebrow><Pill color={C.ember}>{method}{pref === "Auto" ? " - auto" : " - your choice"}</Pill></div>
      ...
      <div style={{ color: C.faint, fontSize: 11.5, marginTop: 8 }}>Auto picks the best fit from your volume ({p.currentWeeklyVolumeKm} km), days ({p.availabilityDays}/wk){vo2 ? ` and VO\u2082 max (${vo2})` : ""}. Or choose one yourself - it changes your weekly quality sessions.</div>
...
function GoalRaceView({ core, dispatch, onBack }) {
  const r = (core.races || []).find((x) => x.priority === "A") || core.races[0]; const [d, setD] = useState({ name: r.name, type: r.type, date: r.date, goalTime: r.goalTime }); const [saved, setSaved] = useState(false);
  return (<div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
...
function ShoesView({ core, dispatch, onBack }) {
  ...
  return (<div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
    <Header title="Shoe tracker" sub={`${shoes.length} pairs - ${totalKm} km logged total`} onBack={onBack} />
    <Card><Eyebrow>Your shoes</Eyebrow><div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      {shoes.map((s) => { const pct = s.lifetimeKm > 0 ? Math.min(100, Math.round((s.km / s.lifetimeKm) * 100)) : 0; const over = s.lifetimeKm > 0 && s.km >= s.lifetimeKm; const col = over ? C.rose : pct >= 80 ? C.amber : C.teal; return (<div key={s.id} style={{ padding: "10px 12px", background: C.surface2, borderRadius: 10, opacity: s.retired ? 0.55 : 1 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><div style={{ fontSize: 13.5, fontWeight: 700 }}>{s.name}{s.retired ? " (retired)" : ""}</div><div style={{ color: col, fontWeight: 800, fontSize: 13, fontVariantNumeric: "tabular-nums" }}>{Math.round(s.km)}{s.lifetimeKm > 0 ? ` / ${s.lifetimeKm}` : ""} km</div></div>
    ...
    <Card><Eyebrow>Add a pair</Eyebrow><div style={{ marginTop: 10 }}>
      ...
      <Field label="Lifetime limit (km, 0 = no limit)" value={n.lifetimeKm} onChange={(v) => setN({ ...n, lifetimeKm: v })} />
      <Field label="Starting mileage (km)" value={n.km} onChange={(v) => setN({ ...n, km: v })} />
...
function SetupView({ core, dispatch, onComplete, onBack }) {
  const p = core.profile, set = (patch) => dispatch({ type: "ProfileUpdated", profile: patch });
  const r = (core.races || []).find((x) => x.priority === "A") || core.races[0];
  ...
  return (<div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
    ...
    <Card><Eyebrow>About you</Eyebrow><div style={{ marginTop: 10 }}>
      ...
      <Field label="Weight (kg)" value={p.weightKg} step="0.1" onChange={(v) => set({ weightKg: v })} />
    ...
    <Card><Eyebrow>Strength 1RMs (kg)</Eyebrow><div style={{ marginTop: 10 }}>{Object.entries({ "Back Squat": "backSquat", "Romanian Deadlift": "romanianDeadlift", "Bench Press": "benchPress", "Overhead Press": "overheadPress" }).map(([name, k]) => (<Field key={k} label={name} value={p.oneRM[k]} step="2.5" onChange={(v) => set({ oneRM: { ...p.oneRM, [k]: v } })} />))}</div><div style={{ color: C.faint, fontSize: 11.5 }}>Used to prescribe strength loads and to protect muscle in race-weight targeting.</div></Card>
...
function runTests() {
  ...
  // Sessions alternate: A = Deadlift + Bench, B = Squat + OHP, with different accessories
  ...
  ok("Session A is Deadlift + Bench", ssA.compound.map((c) => c.name).join("+") === "Romanian Deadlift+Bench Press");
  ok("Session B is Squat + OHP", ssB.compound.map((c) => c.name).join("+") === "Back Squat+Overhead Press");
  ok("Accessories vary between the two sessions", ssA.accessory.map((a) => a.name).join() !== ssB.accessory.map((a) => a.name).join());
  ok("Both sessions still cover all 4 accessory patterns", new Set(ssA.accessory.map((a) => a.category)).size === 4 && new Set(ssB.accessory.map((a) => a.category)).size === 4);
  ...
  const sqW1 = generateStrengthSession(sqProf, 1, "Build", false, 1, {}).compound.find((c) => c.name === "Back Squat");
  const sqW3 = generateStrengthSession(sqProf, 3, "Build", false, 1, {}).compound.find((c) => c.name === "Back Squat");
  ...
  ok("Higher 1RM -> heavier compound prescription", generateStrengthSession({ ...sqProf, oneRM: { ...sqProf.oneRM, backSquat: 160 } }, 1, "Build", false, 1, {}).compound.find((c) => c.name === "Back Squat").weightKg > sqW1.weightKg);
  ...
  const lowBl = lowVol.weeks.slice(0, lowVol.blockWeeks).map((w) => w.longRunKm);
  ok("Low-volume athlete still reaches 32km long", lowBl.some((x) => x >= 32) && lowBl.filter((x) => x >= 30).length >= 2);
...
function TestsView({ onBack }) {
  const results = useMemo(runTests, []); const passed = results.filter((r) => r.pass).length;
  return (<div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
    <Header title="Core logic tests" onBack={onBack} />
    <Card><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><Eyebrow>Engine assertions</Eyebrow><Pill color={passed === results.length ? C.teal : C.rose}>{passed}/{results.length} passing</Pill></div><div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 6 }}>{results.map((r, i) => (<div key={i} style={{ display: "flex", gap: 10, alignItems: "flex-start", padding: "7px 10px", background: C.surface2, borderRadius: 8 }}>{r.pass ? <CheckCircle2 size={16} color={C.teal} style={{ flexShrink: 0, marginTop: 1 }} /> : <AlertTriangle size={16} color={C.rose} style={{ flexShrink: 0, marginTop: 1 }} />}<div><div style={{ fontSize: 13, fontWeight: 600 }}>{r.name}</div>{r.detail && <div style={{ color: C.faint, fontSize: 11.5, fontFamily: "monospace" }}>{r.detail}</div>}</div></div>))}</div></Card>
  </div>);
}
...
export default function App({ storage = null, user = null, onSignOut = null, onChangePassword = null, onDeleteAccount = null }) {
  ...
  const overlay = () => {
    switch (screen?.name) {
      ...
      case "tests": return <TestsView onBack={back} />;
      default: return null;
    }
  };
